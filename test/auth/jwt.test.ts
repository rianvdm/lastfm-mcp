...
describe('JWT Authentication', () => {
	...
	describe('verifySessionToken', () => {
		...
		it('should handle malformed JSON in payload', async () => {
			...
			const header = btoa(JSON.stringify({ alg: 'HS256', typ: 'JWT' }))
				...
				.replace(/\//g, '_')
				.replace(/=/g, '')
			const invalidPayload = btoa('invalid json').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '')
	...
	describe('security properties', () => {
		...
		it('should be resistant to timing attacks', async () => {
			...
			const invalidTokens = ['invalid.token.signature', 'different.invalid.signature', 'yet.another.invalid']
